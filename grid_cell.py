...
class GridCell:
    ...
    def print_char(self, key_char):
        ...
        """
        key_char_text = gc.font.value.render(key_char, True, gc.COLOURS.value["BLACK"])
        gc.SURFACE.value.blit(key_char_text, (self.col * gc.CELL_SIZE.value + gc.CELL_SIZE.value // 2 - key_char_text.get_width() // 2,
                                                    self.row * gc.CELL_SIZE.value + gc.CELL_SIZE.value // 2 - key_char_text.get_height() // 2))
